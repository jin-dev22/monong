...
근데; 저희
김재경 — 오늘 오후 3:58
sql 프리뷰로 확인하고 받으면 됩니다
박민지 — 오늘 오후 3:58
여기서 뭐 들어가야 하나요
이미지
김재경 — 오늘 오후 3:58
monongmonong으로 들어가시면
될거에요
그거 북마크 해놓으면
조금 편합니다
박민지 — 오늘 오후 3:59
...
CREATE TABLE "direct_product" (
	...
	"d_product_created_at"	date	DEFAULT current_date	NULL,
확장
12c917d614fe7b1d.txt
5KB
김재경 — 오늘 오후 4:20
constraint ck_order_status check(order_status in ('P', 'R', 'C', 'D', 'F')),
김재경 — 오늘 오후 4:46
다하면 말씀해주세요!
박민지 — 오늘 오후 4:46
다 했어요
...
constraint pk_direct_review_attachment primary key(d_review_attach_no),
    constraint fk_direct_review_attachment foreign key(d_review_no) 
확장
de7be63fc1910707.txt
2KB
constraint pk_member_direct_order primary key(d_option_no,d_order_no)
constraint pk_member_direct_option_order_no primary key(d_option_no,d_order_no)
onstraint pk_direct_review_attachment_no primary key(d_review_attach_no),
constraint fk_direct_review_no foreign key(d_review_no) 
                                    references direct_review(d_review_no)
ALTER TABLE "direct_review_attachment" ADD CONSTRAINT "PK_DIRECT_REVIEW_ATTACHMENT" PRIMARY KEY (
    "d_review_attach_no"
);
...
constraint fk_direct_option_no foreign key(d_option_no)
                                    references member_direct_order(d_option_no)
constraint fk_direct_order_no foreign key(d_order_no)
                                    references member_direct_order(d_order_no)
...
constraint pk_direct_order_no primary key(d_order_no),
constraint fk_direct_order_member_id foreign key(member_id)
...
constraint pk_direct_product_option_no primary key(d_option_no),
constraint fk_direct_product_option_no foreign key(d_product_no)
                                    references direct_product(d_product_no)
김재경 — 오늘 오후 4:55
CREATE TABLE "direct_product" (
	"d_product_no"	varchar2(100)		NOT NULL,
	"member_id"	varchar2(100)		NOT NULL,
